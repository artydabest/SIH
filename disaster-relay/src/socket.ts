...
/** Broadcast a newly issued (simulated) meteorological warning. */
export function broadcastNewWarning(warning: unknown): void {
  io?.emit("warning:new", warning);
}

/** Broadcast a warning status change (expired, cancelled, reactivated). */
export function broadcastWarningUpdate(warning: unknown): void {
  io?.emit("warning:update", warning);
}

/** Nudge clients that someone's safety status changed (I'm Safe, SOS, check-in). */
export function broadcastSafetyUpdate(): void {
