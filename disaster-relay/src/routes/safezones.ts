...
import {
  recommendShelters,
} from "../services/shelterRecommendation";
...
/**
 * GET /api/safezones/recommend?latitude=&longitude=&warningId=?
 *
 * Explained shelter recommendation. Considers distance, disaster type,
 * severity, shelter suitability and simulated availability — and explicitly
 * tells the client when the user should stay put instead of travelling.
 * Occupancy figures are SIMULATED DEMO data.
 */
router.get("/recommend", async (req, res) => {
  try {
    ...
    const warningId =
      typeof req.query.warningId === "string" ? req.query.warningId : undefined;
    ...
    const result = await recommendShelters(latitude, longitude, warningId);
    return res.json({ success: true, ...result });
  } catch (error) {
    console.error("Error building shelter recommendation:", error);
    return res
      .status(500)
      .json({ success: false, message: "Failed to build recommendation" });
  }
});
...
router.post("/seed-nearby", async (req, res) => {
  ...
  const spots: Array<{
  ...
  }> = [
    {
      ...
      suitableFor: ["FLOOD", "HEAVY_RAINFALL", "CYCLONE", "EARTHQUAKE"],
      accessible: true,
    ...
    },
    {
      ...
      facilities: ["TEMPORARY_ACCOMMODATION", "FOOD", "TOILETS", "DRINKING_WATER", "EMERGENCY_SUPPLIES"],
      suitableFor: ["FLOOD", "CYCLONE", "HEAVY_RAINFALL"],
      accessible: false,
      operatingStatus: "OPEN",
    },
    {
    ...
    },
    {
      ...
      facilities: ["TEMPORARY_ACCOMMODATION", "DRINKING_WATER", "TOILETS"],
      suitableFor: ["FLOOD", "HEAVY_RAINFALL", "THUNDERSTORM"],
      accessible: false,
    ...
    },
    {
      ...
      facilities: ["MEDICAL_ASSISTANCE", "FIRST_AID", "DRINKING_WATER", "FOOD", "CHARGING"],
      suitableFor: ["EARTHQUAKE", "FLOOD", "CYCLONE", "HEATWAVE"],
      accessible: true,
      operatingStatus: "OPEN",
    },
  ];
